...
if user_input:
    x_start, y_start = input(
        "Enter the pixel location where the tiles start, x and y."
    ).split()
    x_start = int(x_start)
    y_start = int(y_start)
    tile_group_width, tile_group_height = input(
        "Enter the width and height of the tile group. (144 by 27 is a good start. "
    ).split()
...
mazeParts = get_pixels_at(
    x_start, y_start, tile_group_width, tile_group_height, tileSheet
)
...
gameGrid = grid(
    xSpot, ySpot, scaleFactor, gameWidth_in_tiles, gameHeight_in_tiles, x_size, y_size
)
...
y_left = 0
font_size = 20
welcomeMessage = textLineObject(
    x_left, y_left, font_size, "Welcome to the Sprite Layer Builder", GREEN, BLUE
)
statusMessage = textLineObject(
    x_left, y_left + (2 * font_size), font_size, "No sprite selected", GREEN, BLACK
)
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if inPalletteGrid == True:
                ...
                if pallette_select == True:
                    ...
                    myTileRectSpot = pygame.Rect(
                        mCol * x_size * scaleFactor,
                        mRow * y_size * scaleFactor,
                        x_size * scaleFactor,
                        y_size * scaleFactor,
                    )
